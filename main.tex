...
\caption{Архитектура ROI-aware конвейера на базе IDet: библиотека объединяет text ROI, face ROI и textured ROI в геометрический выход, а downstream-слой преобразует его в приоритеты для кодировщика.}
...
Практическая политика может задавать разные значения $\alpha$ для разных типов контента. Для лекции с субтитрами допустима небольшая площадь text ROI и высокий qoffset; для обзора товара или fashion-сцены допустима более крупная textured ROI; для видеоконференции приоритетнее лицо, а текстурированные области должны быть ограничены сильнее. Таким образом, $\alpha$ является параметром runtime policy, а не универсальной константой модели.
...
\begin{table}[H]
\centering
\caption{Модули IDet и downstream-слои интеграции с кодировщиком}
...
Downstream ROI integration & Добавление типа ROI, приоритета, area cap и temporal smoothing поверх геометрии \texttt{VecQuad}. & Упорядоченный список ROI metadata для кодировщика. \\
...
6 & Downstream ROI integration & Добавление типа ROI, сортировка, слияние пересечений, area cap и temporal smoothing поверх \texttt{VecQuad}. \\
...
Фактическое представление результата в публичном API IDet минималистично: библиотека возвращает \texttt{VecQuad}, то есть список четырехугольников в координатах изображения. На этапе постобработки конкретные движки используют модельно-зависимые кандидаты с confidence score и, для YOLO, классом объекта, но эти поля не являются частью публичного результата \texttt{Detector::detect}. Для интеграции с кодировщиком одного геометрического контракта недостаточно, поэтому в работе отдельно описывается downstream-запись ROI: она должна добавлять к \texttt{Quad} тип области, приоритет и рекомендуемое смещение QP.
...
\caption{Generic YOLO pipeline для детектора текстурированных ROI: конвертация и аудит датасетов, обучение, AP/confusion analysis, ONNX export, runtime optimization и benchmark/deploy этап.}
\label{fig:generic_pipeline}
\end{figure}

После рисунка важно подчеркнуть границу ответственности pipeline. Он не является частью hot path IDet и не исполняется при обработке кадра. Его результат --- проверенный набор обучающих данных, отчетов и deploy-артефактов, которые затем передаются в C++/ONNX Runtime контур. Благодаря этому IDet остается CPU-first библиотекой без зависимости от Python training framework, а обучение и экспериментальная проверка модели сохраняются как отдельный воспроизводимый процесс.
...
Таксономия также задает границу между ML-частью и видеокодированием. Модель может различать \texttt{dress}, \texttt{skirt} или \texttt{trousers}, но downstream-слой не обязан назначать каждому классу отдельный qoffset. В практической политике эти классы могут попадать в одну группу textured ROI со средним приоритетом, ниже text и face. Поэтому 7-классовая схема используется как обучающая и диагностическая структура, а не как обязательная семантическая схема кодировщика.
...
Более тяжелая модель увеличивает не только время inference, но и давление на кэш, объем intermediate tensors и стоимость работы с памятью. В tiled-режиме это может усиливать backend-bound поведение, уже наблюдаемое в профилировании. Поэтому выигрыш YOLO26s по точности нужно сопоставлять не с одним числом latency модели, а с полной стоимостью кадра: preprocessing, inference, post-processing, слияние ROI и влияние на p99. С этой точки зрения YOLO26n является более безопасной стартовой точкой для CPU-first системы, а INT8-варианты должны рассматриваться как отдельная линия дальнейшей проверки, потому что уменьшение memory footprint сопровождается измеримой потерей detection quality.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\linewidth]{ch07-performance/bench_latency_quality.png}
...
Режим &
\makecell{Inner\\with, \%} &
\makecell{Inner\\without, \%} &
\makecell{Прирост,\\п.п.} &
\makecell{Total\\with, GB} &
\makecell{Total\\without, GB} \\
\midrule
...
\begin{lstlisting}[caption={Сокращенный пример подключения cloth/YOLO режима через публичный API IDet},label={lst:textured_detector_contract}]
idet::RuntimePolicy policy{};
policy.ort_intra_threads = 8;
policy.ort_inter_threads = 1;
policy.tile_omp_threads = 8;
...
auto detector_result = idet::Detector::create(cfg);
...
\bibitem{idet_model_zoo} Parshin D. IDet model zoo: text, face and clothing detector artifacts. URL: \url{https://github.com/De-Par/IDet/blob/main/docs/model-zoo.md}.
